...
<title>Solicitações | Plataforma ITSM</title>
...
<div class="container-fluid ps-3">
  ...
  <div class="navbar-title bg-black text-white px-2 py-0 rounded custom-border">
    <?php if(isset($_GET['criado_por']) && $_GET['criado_por'] == 'me'): ?>
      Minhas Solicitações
    <?php else: ?>
      Lista Geral
    <?php endif; ?>
...
$filtrar_me = isset($_GET['criado_por']) && $_GET['criado_por'] == 'me';
...
(
  SELECT 
    incidentes.codigo, 
    incidentes.titulo, 
    'Incidente' AS tipo_solicitacao,
    incidentes.prioridade AS prioridade_tipo, 
    status.nome AS status, 
    incidentes.descricao, 
    criador.nome AS nome_criador,
    atendente.nome AS nome_atendente,
    incidentes.criado_em
  FROM incidentes
  LEFT JOIN usuarios AS criador ON incidentes.criado_por = criador.id
  LEFT JOIN usuarios AS atendente ON incidentes.atribuido_para = atendente.id
  LEFT JOIN status ON incidentes.status = status.id
  WHERE (incidentes.codigo LIKE '%$pesquisa%' OR incidentes.titulo LIKE '%$pesquisa%' OR incidentes.descricao LIKE '%$pesquisa%')
";
if ($filtrar_me || $_SESSION["usuario_funcao"] == "Solicitante") {
  $sql .= " AND incidentes.criado_por = '$usuario_id' ";
}
$sql .= ")
UNION ALL
(
  SELECT 
    requisicoes.codigo, 
    requisicoes.titulo, 
    'Requisição' AS tipo_solicitacao,
    requisicoes.tipo AS prioridade_tipo, 
    status.nome AS status, 
    requisicoes.descricao, 
    criador.nome AS nome_criador,
    atendente.nome AS nome_atendente,
    requisicoes.criado_em
  FROM requisicoes
  ...
  WHERE (requisicoes.codigo LIKE '%$pesquisa%' OR requisicoes.titulo LIKE '%$pesquisa%' OR requisicoes.descricao LIKE '%$pesquisa%')
";
if ($filtrar_me || $_SESSION["usuario_funcao"] == "Solicitante") {
  $sql .= " AND requisicoes.criado_por = '$usuario_id' ";
}
$sql .= ")
...
foreach ($resultado as $linha) {
    echo "
    <tr>
        <td><a href='/ITSM/formulario/" . ($linha['tipo_solicitacao'] == 'Incidente' ? 'form_incidente' : 'form_requisicao') . ".php?codigo=" . $linha['codigo'] . "' class='link-dark'>" . $linha['codigo'] . "</a></td>
        <td>" . $linha['titulo'] . "</td>
        <td><span class='badge bg-" . ($linha['tipo_solicitacao'] == 'Incidente' ? 'danger' : 'secondary') . "'>" . $linha['tipo_solicitacao'] . "</span></td>
        <td><span class='badge bg-" . 
            ($linha['tipo_solicitacao'] == 'Incidente' ? 
                ($linha['prioridade_tipo'] == 'Baixa' ? 'success' : ($linha['prioridade_tipo'] == 'Média' ? 'warning' : ($linha['prioridade_tipo'] == 'Alta' ? 'danger' : 'dark')))
                : 'secondary'
            ) . "'>" . $linha['prioridade_tipo'] . "</span></td>
        <td><span class='badge bg-" . 
            ($linha['status'] == 'Aberto' ? 'primary' : ($linha['status'] == 'Em andamento' ? 'warning' : ($linha['status'] == 'Pendente' ? 'secondary' : ($linha['status'] == 'Em análise' ? 'info' : ($linha['status'] == 'Concluído' || $linha['status'] == 'Resolvido' ? 'success' : ($linha['status'] == 'Cancelado' ? 'danger' : 'dark'))))))
        . "'>" . $linha['status'] . "</span></td>
        <td>" . $linha['descricao'] . "</td>
        <td>" . htmlspecialchars($linha['nome_atendente']) . "</td>
        <td>" . htmlspecialchars($linha['nome_criador']) . "</td>
        <td>" . $linha['criado_em'] . "</td>
        <td>
            <a href='/ITSM/formulario/" . ($linha['tipo_solicitacao'] == 'Incidente' ? 'form_incidente' : 'form_requisicao') . ".php?codigo=" . $linha['codigo'] . "' class='btn btn-sm btn-outline-primary'>
                <i class='bi bi-pencil-square'></i>
            </a>
        </td>";
    if($_SESSION["usuario_funcao"] == "Administrador"){
      echo "<td>
              <button class='btn btn-sm btn-outline-danger' onclick=\"confirmarExclusao('" . $linha['codigo'] . "')\">
...
function confirmarExclusao(codigo) {
    ...
    if (codigo.includes('INC')) {
        confirmarBtn.href = '/ITSM/lista/deletar_registro.php?codigo_incidente=' + encodeURIComponent(codigo);
    } else {
