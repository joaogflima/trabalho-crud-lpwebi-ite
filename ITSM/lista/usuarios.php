...
<title>Usuários | Plataforma ITSM</title>
...
foreach ($resultado as $linha) {
    ...
    <tr>
        ...
        <td><span class='badge bg-dark'>" . htmlspecialchars($linha['funcao']) . "</span></td>
        <td>" . $status . "</td>
        <td>{$linha['criado_em']}</td>
        <td>
            <a href='/ITSM/formulario/form_usuario.php?id=" . $linha['id'] . "' class='btn btn-sm btn-outline-primary'>
                <i class='bi bi-pencil-square'></i>
            </a>
        </td>
        <td>
            <button type='button' class='btn btn-sm btn-outline-danger' onclick='confirmarExclusao(\"{$linha['id']}\")'>
...
<div class="modal fade" id="confirmarDelete" tabindex="-1" aria-labelledby="confirmarDeleteLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">Você tem certeza que deseja excluir este usuário?</div>
...
function confirmarExclusao(id) {
  document.getElementById('confirmarExcluirBtn').href = '/ITSM/lista/deletar_registro.php?usuario=' + encodeURIComponent(id);
  new bootstrap.Modal(document.getElementById('confirmarDelete')).show();
}
</script>
