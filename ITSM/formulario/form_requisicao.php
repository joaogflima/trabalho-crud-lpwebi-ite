...
$isReadOnly = (in_array($requisicao['status'], [7, 8, 4]) && $_SESSION["usuario_funcao"] != "Administrador") || ($_SESSION["usuario_funcao"] == "Solicitante" && !empty($codigo));
...
<head>
    ...
    <title>Requisição | Plataforma ITSM</title>
...
<nav class="navbar navbar-expand-lg">
  <div class="container-fluid ps-3">
    ...
    <div class="navbar-title bg-black text-white px-2 py-0 rounded custom-border">
        <?php echo $codigo ? "Requisição | " . htmlspecialchars($codigo) : "Requisição"; ?>
    </div>
...
<div class="form-container">
    ...
    <h2 class="text-center mb-4"><?php echo $codigo ? 'Atualizar Requisição' : 'Nova Requisição'; ?></h2>

    <form id="formRequisicao" method="POST" action="<?php echo $codigo ? '/ITSM/formulario/atualiza_registro.php' : '/ITSM/formulario/cria_registro.php'; ?>">
        
        <input type="hidden" name="tipo" value="requisicao">
        ...
        <?php if ($codigo): ?>
            ...
            <div class="mb-3">
                ...
                <input type="hidden" name="codigo_requisicao" value="<?php echo htmlspecialchars($codigo); ?>">
            </div>

            <div class="mb-3">
              <label class="form-label">Status</label>
              <select class="form-select" name="status" id="status" required <?= $isReadOnly ? 'disabled' : '' ?>>
                  <option value="1" <?= $requisicao['status'] == 1 ? 'selected' : ''; ?>>Aberto</option>
                  <option value="6" <?= $requisicao['status'] == 6 ? 'selected' : ''; ?>>Em análise</option>
                  <option value="2" <?= $requisicao['status'] == 2 ? 'selected' : ''; ?>>Em andamento</option>
                  <option value="5" <?= $requisicao['status'] == 5 ? 'selected' : ''; ?>>Pendente</option>
                  <option value="3" <?= $requisicao['status'] == 3 ? 'selected' : ''; ?>>Resolvido</option>
                  <option value="7" <?= $requisicao['status'] == 7 ? 'selected' : ''; ?>>Concluído</option>
                  <option value="8" <?= $requisicao['status'] == 8 ? 'selected' : ''; ?>>Cancelado</option>
              </select>
              <?php if ($isReadOnly): ?>
                <input type="hidden" name="status" value="<?= $requisicao['status'] ?>">
              <?php endif; ?>
            </div>
            ...
            <?php if ($_SESSION["usuario_funcao"] == "Atendente" || $_SESSION["usuario_funcao"] == "Administrador"): ?>
              <div class="mb-3">
                ...
                <select class="form-select" name="atribuido_para" id="atribuido_para" <?= $isReadOnly ? 'disabled' : '' ?>>
                  ...
                  ?>
                    <option value="<?= $u['id'] ?>" <?= $requisicao['atribuido_para'] == $u['id'] ? 'selected' : '' ?>>
                      <?= htmlspecialchars($u['nome']) ?>
                    </option>
                  <?php endwhile; ?>
                </select>
                <?php if ($isReadOnly): ?>
                  <input type="hidden" name="atribuido_para" value="<?= $requisicao['atribuido_para'] ?>">
                <?php endif; ?>
              </div>
            <?php endif; ?>
        <?php endif; ?>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="titulo" id="titulo" required placeholder="Digite o título da requisição..." value="<?php echo htmlspecialchars($requisicao['titulo']); ?>" <?= $isReadOnly ? 'readonly' : '' ?>>
        </div>

        <div class="mb-3">
            <label class="form-label">Descrição</label>
            <textarea class="form-control" name="descricao" id="descricao" rows="4" required placeholder="Descreva a solicitação..." <?= $isReadOnly ? 'readonly' : '' ?>><?php echo htmlspecialchars($requisicao['descricao']); ?></textarea>
        </div>

        <div class="mb-3">
          <label class="form-label">Tipo de Requisição</label>
          <select class="form-select" name="tipo_requisicao" id="tipo_requisicao" required <?= $isReadOnly ? 'disabled' : '' ?>>
            <option value="">Selecione o tipo</option>
            <option value="Acesso" <?= $requisicao['tipo'] == 'Acesso' ? 'selected' : ''; ?>>Acesso</option>
            <option value="Hardware" <?= $requisicao['tipo'] == 'Hardware' ? 'selected' : ''; ?>>Hardware</option>
            <option value="Software" <?= $requisicao['tipo'] == 'Software' ? 'selected' : ''; ?>>Software</option>
            <option value="Outro" <?= $requisicao['tipo'] == 'Outro' ? 'selected' : ''; ?>>Outro</option>
          </select>
          <?php if ($isReadOnly): ?>
            <input type="hidden" name="tipo_requisicao" value="<?= $requisicao['tipo'] ?>">
          <?php endif; ?>
        </div>
        ...
        <div class="d-flex justify-content-center flex-wrap gap-3 mt-4">
            <div class="d-flex gap-3">
              <?php
                $isCriador = $_SESSION['usuario_id'] == $requisicao['criado_por'];
                $permitido = ($_SESSION['usuario_funcao'] == 'Administrador' && isset($_GET['codigo'])) || $isCriador;
                $statusAtual = (int)$requisicao['status'];
              ?>
